...
WHERE age > 20  
```
Once you've used the `USE` operator, it is effectively like prepending whatever namespace to your `FROM` clause.  So the query above, is functionally equivalent to the query below.  Note that the `USE` operator only works in the command line interface and has no effect in web sessions.
```
SELECT * 
...
| $53626.04 | VARCHAR |
 ...
 In order to fix this problem, you will need to tell Drill what data type the `salary` field is, and in order to use a comparison operator both sides of the equation will need to be numeric, and to convert the field to a numeric data type, you will need to use the `CAST( <field> AS <data type> )` function. 
...
This query will unfortunately also fail because there are extraneous characters such as the `$` in the `Gross 
